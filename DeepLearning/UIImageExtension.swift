...
//    // Get grayscale image from normal image.
    func getFilledImage(fillWindow:Array2D) -> UIImage? {
        ...
        for var x = 0; x < Int(pixelsWide) ; x++ {
            for var y = 0; y < Int(pixelsHigh) ; y++ {
                ...
                var fillValue = Int(floor(fillWindow[x,y]*Float(255.0)))
                fillValue += 50
                if (fillValue < 0)
                {
                    fillValue = 0
                ...
                let avg = UInt32(fillValue)

                dataType[offset + 1] = UInt8(avg) // set R
                dataType[offset + 2] = UInt8(avg) // set G
                dataType[offset + 3] = UInt8(avg) // set B
            }
        }
    ...
    */
    ...
    /*
    Get pixel alpha only for a pixel in the image.
    */
    
    // Nifty trick -- UIColor can return the greyscale value with getWhite()
    func getPixelAlphaAtLocation(point:CGPoint)->CGFloat?
    {
        self.sanitizePoint(point)
        ...
        return CGFloat(alphaValue)/255.0
    }
